...
export function EchangesAdmin({ groupId, cycleId, echanges }: Props) {
  ...
  const action = async (id_demande: string, act: string) => {
    ...
    try {
      const res = await fetch(
        `/api/groups/${groupId}/cycles/${cycleId}/echanges/${id_demande}`,
        {
          method: "PATCH",
  ...
  const autres = echanges.filter((e) => e.statut !== "ACCEPTEE_MEMBRES");

  if (echanges.length === 0) {
    return (
      <p className="text-sm text-gray-500 italic">Aucune demande d&apos;échange sur ce cycle.</p>
    );
  }
    ...
    <div className="space-y-4">
      ...
      {enAttente.length > 0 && (
        <div className="space-y-3">
          ...
          {enAttente.map((e) => (
            <div key={e.id_demande} className="rounded-xl border border-purple-200 bg-purple-50 p-4 dark:border-purple-800 dark:bg-purple-900/20">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
                    <strong>{e.demandeur.user.prenom} {e.demandeur.user.nom}</strong>
                    <ArrowLeftRight className="inline h-3.5 w-3.5 mx-1.5 text-purple-500" />
                    <strong>{e.cible.user.prenom} {e.cible.user.nom}</strong>
                  </p>
      ...
      {autres.length > 0 && (
        <div className="space-y-2">
          ...
          {autres.map((e) => {
            const s = STATUT_LABELS[e.statut] ?? { label: e.statut, color: "bg-gray-100 text-gray-500" };
            return (
              <div key={e.id_demande} className="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900 flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm text-gray-700 dark:text-gray-300">
                    <strong>{e.demandeur.user.prenom} {e.demandeur.user.nom}</strong>
                    {" ↔ "}
                    <strong>{e.cible.user.prenom} {e.cible.user.nom}</strong>
                    <span className="ml-2 text-xs text-gray-400">(tour {e.tour_demandeur} ↔ {e.tour_cible})</span>
                  </p>
                </div>
