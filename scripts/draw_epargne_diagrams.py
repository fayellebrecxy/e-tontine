...
# 1. DSS Ouvrir un compte
def generate_dss_ouvrir_compte():
    ...
    draw_arrow(draw, 180, 160, 520, 160, "1. ouvrirCompteÉpargne(idMembre)", F_REG, DARK_BLUE, "top")
    draw_arrow(draw, 520, 240, 180, 240, "2. compteÉpargneOuvert", F_REG, DARK_BLUE, "bottom", dashed=True)
    
    img.save("Docs/dss-ouvrir-compte.png", "PNG")
    print("Saved Docs/dss-ouvrir-compte.png")

# 2. DSS Effectuer un dépôt
def generate_dss_effectuer_depot():
    img = Image.new("RGB", (700, 360), BG_COLOR)
    draw = ImageDraw.Draw(img)
    ...
    draw_arrow(draw, 180, 160, 520, 160, "1. effectuerDépôt(idCompte, montant)", F_REG, DARK_BLUE, "top")
    draw_arrow(draw, 520, 240, 180, 240, "2. soldeÉpargneMisÀJour", F_REG, DARK_BLUE, "bottom", dashed=True)
    
    img.save("Docs/dss-effectuer-depot.png", "PNG")
    print("Saved Docs/dss-effectuer-depot.png")

# 3. Activité Ouvrir un compte
def generate_activite_ouvrir_compte():
    ...
    rounded_box(draw, 350, 120, 280, 60, "Accéder au module Épargne\net initier l'ouverture de compte")
    draw_arrow(draw, 350, 150, 350, 200)
    
    diamond(draw, 350, 250, 180, 100, "Compte existe déjà ?")
    
    # Yes branch
    draw.line((260, 250, 130, 250), fill=DARK_BLUE, width=2)
    draw.text((195, 230), "Oui", font=F_BOLD, fill=DARK_BLUE, anchor="ma")
    rounded_box(draw, 130, 170, 180, 50, "Afficher message d'erreur\n(compte déjà actif)", F_REG, DEC_BORDER, DEC_BG)
    draw.line((130, 195, 130, 465), fill=DARK_BLUE, width=2)
    draw_arrow(draw, 130, 465, 335, 465)
    
    # No branch
    draw_arrow(draw, 350, 300, 350, 360, "Non", F_BOLD, DARK_BLUE, "bottom")
    
    rounded_box(draw, 350, 390, 280, 60, "Créer le livret d'épargne et\nactiver le compte pour le membre", F_REG, GREEN_BORDER, GREEN_BG)
    draw_arrow(draw, 350, 420, 350, 450)
    
    final_node(draw, 350, 465)
    
    img.save("Docs/activite-ouvrir-compte.png", "PNG")
    print("Saved Docs/activite-ouvrir-compte.png")

# 4. Activité Effectuer un dépôt
def generate_activite_effectuer_depot():
    img = Image.new("RGB", (700, 510), BG_COLOR)
    draw = ImageDraw.Draw(img)
    ...
    rounded_box(draw, 350, 120, 280, 60, "Sélectionner le compte épargne\net saisir le montant à déposer")
    draw_arrow(draw, 350, 150, 350, 200)
    
    diamond(draw, 350, 250, 180, 100, "Montant valide ?")
    ...
    rounded_box(draw, 350, 390, 280, 60, "Initier le paiement et mettre\nà jour le solde d'épargne", F_REG, GREEN_BORDER, GREEN_BG)
    draw_arrow(draw, 350, 420, 350, 450)
    
    final_node(draw, 350, 465)
    
    img.save("Docs/activite-effectuer-depot.png", "PNG")
    print("Saved Docs/activite-effectuer-depot.png")

if __name__ == "__main__":
    generate_dss_ouvrir_compte()
    generate_dss_effectuer_depot()
    generate_activite_ouvrir_compte()
    generate_activite_effectuer_depot()
