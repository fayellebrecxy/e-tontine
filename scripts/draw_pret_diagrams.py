...
# 1. DSS Demander un prêt
def generate_dss_demander_pret():
    ...
    draw_arrow(draw, 180, 160, 520, 160, "1. demanderPrêt(montant, durée, avalistes)", F_REG, DARK_BLUE, "top")
    draw_arrow(draw, 520, 240, 180, 240, "2. demandeEnregistrée / refusÉligibilité", F_REG, DARK_BLUE, "bottom", dashed=True)
    
    img.save("Docs/dss-demander-pret.png", "PNG")
    print("Saved Docs/dss-demander-pret.png")

# 2. DSS Accepter une garantie
def generate_dss_accepter_garantie():
    img = Image.new("RGB", (700, 360), BG_COLOR)
    draw = ImageDraw.Draw(img)
    ...
    draw_arrow(draw, 180, 160, 520, 160, "1. accepterGarantie(idPrêt)", F_REG, DARK_BLUE, "top")
    draw_arrow(draw, 520, 240, 180, 240, "2. garantieConfirmée", F_REG, DARK_BLUE, "bottom", dashed=True)
    
    img.save("Docs/dss-accepter-garantie.png", "PNG")
    print("Saved Docs/dss-accepter-garantie.png")

# 3. Activité Demander un prêt
def generate_activite_demander_pret():
    ...
    rounded_box(draw, 350, 390, 280, 60, "Enregistrer la demande de prêt\net notifier les garants désignés", F_REG, GREEN_BORDER, GREEN_BG)
    draw_arrow(draw, 350, 420, 350, 450)
    
    final_node(draw, 350, 465)
    
    img.save("Docs/activite-demander-pret.png", "PNG")
    print("Saved Docs/activite-demander-pret.png")

# 4. Activité Accepter une garantie
def generate_activite_accepter_garantie():
    img = Image.new("RGB", (700, 510), BG_COLOR)
    draw = ImageDraw.Draw(img)
    ...
    rounded_box(draw, 350, 120, 280, 60, "Consulter la demande de garantie\nreçue pour un prêt")
    draw_arrow(draw, 350, 150, 350, 200)
    
    diamond(draw, 350, 250, 180, 100, "Accepter la demande ?")
    
    # No branch
    draw.line((260, 250, 130, 250), fill=DARK_BLUE, width=2)
    draw.text((195, 230), "Non", font=F_BOLD, fill=DARK_BLUE, anchor="ma")
    rounded_box(draw, 130, 170, 180, 50, "Refuser d'être garant et\nnotifier le demandeur", F_REG, DEC_BORDER, DEC_BG)
    draw.line((130, 195, 130, 465), fill=DARK_BLUE, width=2)
    draw_arrow(draw, 130, 465, 335, 465)
    ...
    rounded_box(draw, 350, 390, 280, 60, "Valider la garantie, notifier\nle demandeur et mettre à jour le prêt", F_REG, GREEN_BORDER, GREEN_BG)
    draw_arrow(draw, 350, 420, 350, 450)
    
    final_node(draw, 350, 465)
    
    img.save("Docs/activite-accepter-garantie.png", "PNG")
    print("Saved Docs/activite-accepter-garantie.png")

if __name__ == "__main__":
    generate_dss_demander_pret()
    generate_dss_accepter_garantie()
    generate_activite_demander_pret()
    generate_activite_accepter_garantie()
