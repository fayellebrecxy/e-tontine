...
# 1. DSS Rejoindre un groupe
def generate_dss_rejoindre_groupe():
    ...
    draw_arrow(draw, 180, 160, 520, 160, "1. rejoindreGroupe(codeInvitation)", F_REG, DARK_BLUE, "top")
    draw_arrow(draw, 520, 240, 180, 240, "2. adhésionConfirmée / codeInvalide", F_REG, DARK_BLUE, "bottom", dashed=True)
    
    img.save("Docs/dss-rejoindre-groupe.png", "PNG")
    print("Saved Docs/dss-rejoindre-groupe.png")

# 2. DSS Exclure un membre
def generate_dss_exclure_membre():
    img = Image.new("RGB", (700, 360), BG_COLOR)
    draw = ImageDraw.Draw(img)
    ...
    draw_arrow(draw, 180, 160, 520, 160, "1. exclureMembre(idMembre)", F_REG, DARK_BLUE, "top")
    draw_arrow(draw, 520, 240, 180, 240, "2. membreExclu / accèsRefusé", F_REG, DARK_BLUE, "bottom", dashed=True)
    
    img.save("Docs/dss-exclure-membre.png", "PNG")
    print("Saved Docs/dss-exclure-membre.png")

# 3. Activité Rejoindre un groupe
def generate_activite_rejoindre_groupe():
    ...
    # Box 1: Saisir le code
    rounded_box(draw, 350, 120, 280, 60, "Saisir le code d'invitation\nreçu pour le groupe")
    draw_arrow(draw, 350, 150, 350, 200)
    
    # Diamond 1: Code valide
    diamond(draw, 350, 250, 180, 100, "Code d'invitation\nvalide ?")
    ...
    draw_arrow(draw, 130, 250, 130, 195)  # Vertical arrow pointing up to the bottom of error box
    draw.line((130, 145, 130, 120), fill=DARK_BLUE, width=2)
    draw_arrow(draw, 130, 120, 210, 120)  # Connects back to left of Box 1
    
    # Yes branch: Add member
    draw_arrow(draw, 350, 300, 350, 360, "Oui", F_BOLD, DARK_BLUE, "bottom")
    
    rounded_box(draw, 350, 390, 280, 60, "Ajouter le membre au groupe\navec le statut actif", F_REG, GREEN_BORDER, GREEN_BG)
    draw_arrow(draw, 350, 420, 350, 465)  # Touches top of final node
    
    final_node(draw, 350, 478)
    
    img.save("Docs/activite-rejoindre-groupe.png", "PNG")
    print("Saved Docs/activite-rejoindre-groupe.png")

# 4. Activité Exclure un membre
def generate_activite_exclure_membre():
    img = Image.new("RGB", (700, 510), BG_COLOR)
    draw = ImageDraw.Draw(img)
    ...
    # Box 1: Select member
    rounded_box(draw, 350, 120, 280, 60, "Sélectionner le membre et\ndemander l'exclusion du groupe")
    draw_arrow(draw, 350, 150, 350, 200)
    
    # Diamond 1: Is Admin?
    diamond(draw, 350, 250, 180, 100, "Est administrateur ?")
    
    # No branch: Fails, goes to Final Node
    draw.line((260, 250, 130, 250), fill=DARK_BLUE, width=2)
    draw.text((195, 230), "Non", font=F_BOLD, fill=DARK_BLUE, anchor="ma")
    
    rounded_box(draw, 130, 320, 180, 60, "Afficher message d'erreur\n(autorisation requise)", F_REG, DEC_BORDER, DEC_BG)
    draw_arrow(draw, 130, 250, 130, 290)  # Arrow pointing down to Error Box top
    draw.line((130, 350, 130, 465), fill=DARK_BLUE, width=2)
    draw_arrow(draw, 130, 465, 337, 465)  # Connects directly to left of final node
    
    # Yes branch: Exclude member
    draw_arrow(draw, 350, 300, 350, 360, "Oui", F_BOLD, DARK_BLUE, "bottom")
    
    rounded_box(draw, 350, 390, 280, 60, "Désactiver le membre dans le\ngroupe et lui envoyer une notification", F_REG, GREEN_BORDER, GREEN_BG)
    draw_arrow(draw, 350, 420, 350, 452)  # Touches top of final node (465 - 13)
    
    final_node(draw, 350, 465)
    
    img.save("Docs/activite-exclure-membre.png", "PNG")
    print("Saved Docs/activite-exclure-membre.png")

if __name__ == "__main__":
    generate_dss_rejoindre_groupe()
    generate_dss_exclure_membre()
    generate_activite_rejoindre_groupe()
    generate_activite_exclure_membre()
