...
# 1. DSS Créer un cycle
def generate_dss_creer_cycle():
    ...
    draw_arrow(draw, 180, 160, 520, 160, "1. créerCycle(montant, tours, participants)", F_REG, DARK_BLUE, "top")
    draw_arrow(draw, 520, 240, 180, 240, "2. cycleCréé", F_REG, DARK_BLUE, "bottom", dashed=True)
    
    img.save("Docs/dss-creer-cycle.png", "PNG")
    print("Saved Docs/dss-creer-cycle.png")

# 2. DSS Payer une cotisation
def generate_dss_payer_cotisation():
    img = Image.new("RGB", (700, 360), BG_COLOR)
    draw = ImageDraw.Draw(img)
    ...
    draw_arrow(draw, 180, 160, 520, 160, "1. payerCotisation(idCycle, montant)", F_REG, DARK_BLUE, "top")
    draw_arrow(draw, 520, 240, 180, 240, "2. cotisationEnregistrée", F_REG, DARK_BLUE, "bottom", dashed=True)
    
    img.save("Docs/dss-payer-cotisation.png", "PNG")
    print("Saved Docs/dss-payer-cotisation.png")

# 3. Activité Créer un cycle
def generate_activite_creer_cycle():
    ...
    rounded_box(draw, 350, 120, 280, 60, "Saisir les informations du cycle\n(montant, nombre de tours)")
    ...
    rounded_box(draw, 350, 390, 280, 60, "Créer le cycle en base de données\net notifier les membres", F_REG, GREEN_BORDER, GREEN_BG)
    draw_arrow(draw, 350, 420, 350, 450)
    
    final_node(draw, 350, 465)
    
    img.save("Docs/activite-creer-cycle.png", "PNG")
    print("Saved Docs/activite-creer-cycle.png")

# 4. Activité Payer une cotisation
def generate_activite_payer_cotisation():
    img = Image.new("RGB", (700, 510), BG_COLOR)
    draw = ImageDraw.Draw(img)
    ...
    rounded_box(draw, 350, 120, 280, 60, "Sélectionner le cycle actif\net initier le versement")
    draw_arrow(draw, 350, 150, 350, 200)
    
    diamond(draw, 350, 250, 180, 100, "Montant disponible ?")
    
    # No branch
    draw.line((260, 250, 130, 250), fill=DARK_BLUE, width=2)
    draw.text((195, 230), "Non", font=F_BOLD, fill=DARK_BLUE, anchor="ma")
    rounded_box(draw, 130, 170, 180, 50, "Afficher message d'erreur\n(fonds insuffisants)", F_REG, DEC_BORDER, DEC_BG)
    draw.line((130, 195, 130, 465), fill=DARK_BLUE, width=2)
    draw_arrow(draw, 130, 465, 335, 465)
    ...
    rounded_box(draw, 350, 390, 280, 60, "Enregistrer la cotisation et\nmettre à jour le solde du cycle", F_REG, GREEN_BORDER, GREEN_BG)
    draw_arrow(draw, 350, 420, 350, 450)
    
    final_node(draw, 350, 465)
    
    img.save("Docs/activite-payer-cotisation.png", "PNG")
    print("Saved Docs/activite-payer-cotisation.png")

if __name__ == "__main__":
    generate_dss_creer_cycle()
    generate_dss_payer_cotisation()
    generate_activite_creer_cycle()
    generate_activite_payer_cotisation()
